...
export const Game = ({ fastForwardEvents }: PropsType) => {
  ...
  const Screen = () => {
    if (state.matches("home")) {
      return (
          <HomeScreen onStartButtonClick={() => send("START_BUTTON_CLICKED")}/>
      );
    }
    ...
    if (state.matches("playing")) {
      if (state.matches("playing.level1")) {
        return (
            <>
              <LevelBackgroundImage src={level1BackgroundPng} alt="Dungeon room"/>
  ...
  return  <ScreenTransition key={JSON.stringify(state.value)}>
    <Screen />
  </ScreenTransition>
};
